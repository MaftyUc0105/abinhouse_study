...
**备份到微信（推荐，不需要任何账号）**：设置 → 备份 → "准备备份文件" → "分享到微信 / 网盘"，在分享菜单里选微信 →"文件传输助手"。超过 7 天没有备份也没有同步时，首页会提醒。

- 安卓 Chrome 的系统分享不允许分享 ZIP，所以分享出去的是 `abinhouse_backup_YYYY-MM-DD.txt`（JSON 文本，照片以 base64 内嵌），比 ZIP 大约三分之一。
- 恢复：在微信里打开备份文件并"保存到手机"，再到 设置 → 导入备份 选择它。支持 .txt 和 .zip。
- 分两步：先点"准备备份文件"，再点"分享到微信 / 网盘"或"保存到手机"。浏览器要求分享必须紧跟点击，所以不能一步完成。
- 面板底部的"检测"一行显示浏览器是否支持分享文件；备份失败时原因直接显示在按钮下方。

**导出 ZIP**：设置 → "导出 ZIP 到下载"，含 `backup.json` 和 `images/*.jpg`。
